import 'package:flutter_riverpod/flutter_riverpod.dart';

import '../../../core/auth/auth_provider.dart';
import '../../../core/context/app_context.dart';
import '../../../core/context/context_switch_provider.dart';
import '../../../core/errors/result.dart';
import '../../tasks/data/task_model.dart';
import '../../tasks/presentation/task_list_provider.dart';
import '../data/project_model.dart';
import '../data/project_repository.dart';

/// Datenschicht-Zugriff für Projekte.
final Provider<ProjectRepository> projectRepositoryProvider =
    Provider<ProjectRepository>((ref) {
      return ProjectRepository(ref.read(apiClientProvider));
    });

/// Lädt die Projektliste passend zum globalen Kontext-Schalter (siehe
/// [contextSwitchProvider]/ADR-0006): `alles` zeigt unfiltert, `privat`/
/// `arbeit` filtern serverseitig.
class ProjectListNotifier extends AsyncNotifier<List<Project>> {
  @override
  Future<List<Project>> build() async {
    final context = ref.watch(contextSwitchProvider);
    final result = await ref.read(projectRepositoryProvider).list(
      kontext: context == AppContext.alles ? null : context.name,
    );
    return switch (result) {
      Ok(:final value) => value,
      Err(:final failure) => throw failure,
    };
  }

  /// Lädt die Liste neu — z. B. nach Pull-to-Refresh oder einer Änderung,
  /// die den aktuellen Datenstand veraltet.
  Future<void> refresh() async {
    ref.invalidateSelf();
    await future;
  }
}

/// Aktuelle Projektliste, gefiltert nach dem globalen Kontext, siehe
/// [ProjectListNotifier].
///
/// `retry: null` deaktiviert Riverpods automatisches Wiederholen mit
/// exponentiellem Backoff: [Failure]s sind bereits übersetzte, fachliche
/// Fehler (kein `Error`), die sonst bis zu zehnmal im Hintergrund erneut
/// versucht würden, bevor sie als [AsyncError] sichtbar werden — die Nutzer
/// sähen minutenlang nur einen Ladeindikator statt der Fehlermeldung.
final AsyncNotifierProvider<ProjectListNotifier, List<Project>>
projectListProvider = AsyncNotifierProvider<ProjectListNotifier, List<Project>>(
  ProjectListNotifier.new,
  retry: (_, _) => null,
);

/// Aufgaben eines einzelnen Projekts (`GET /tasks?projekt_id=`) — Grundlage
/// für den Fortschrittsbalken in der Projektdetailansicht. Eigene Familie
/// statt clientseitigem Filtern der globalen Aufgabenliste, weil der
/// globale Kontext-Filter sonst Aufgaben anderer Kontexte ausblenden würde,
/// auch wenn sie zu diesem Projekt gehören.
///
/// `retry: null` aus demselben Grund wie bei [projectListProvider].
final projectTasksProvider =
    FutureProvider.family<List<Task>, String>((ref, projektId) async {
      final result = await ref
          .read(taskRepositoryProvider)
          .list(projektId: projektId);
      return switch (result) {
        Ok(:final value) => value,
        Err(:final failure) => throw failure,
      };
    }, retry: (_, _) => null);
